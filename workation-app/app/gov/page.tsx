...
const GANGWON_SUBSIDY_AMOUNT = 100000
...
function isGangwon(region: string | undefined) {
  return !!(region?.includes('강원'))
}

function cityOf(region: string | undefined): string {
  if (!region) return '기타'
...
export default function GovPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      if (bData) {
        setBookings((bData as Booking[]).filter(b => isGangwon(b.accommodations?.region ?? undefined)))
      }
      if (uData) {
  ...
  // 시군별 동적 집계
  const cityMap: Record<string, Booking[]> = {}
  active.forEach(b => {
  ...
  const cityRows = Object.entries(cityMap)
    .sort((a, b) => b[1].reduce((s, x) => s + (x.guests || 0), 0) - a[1].reduce((s, x) => s + (x.guests || 0), 0))
  ...
  // 지원금 예산
  const subsidyUsed = confirmed.reduce((s, b) => s + (b.guests || 0) * GANGWON_SUBSIDY_AMOUNT, 0)
  const budgetPct   = Math.min(100, Math.round((subsidyUsed / GANGWON_BUDGET) * 100))
    ...
    <div className="min-h-screen bg-[#F8FAFC]">
      <header className="sticky top-0 z-50 bg-[#F8FAFC]/90 backdrop-blur border-b border-[#E2E8F0] px-6 h-14 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm text-[#475569] font-medium">강원특별자치도</span>
        </div>
        <div className="flex items-center gap-3">
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        <div className="mb-8">
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="text-2xl font-black">강원특별자치도 워케이션 현황</h1>
          </div>
          <p className="text-sm text-[#475569]">실시간 데이터 · 강원도 전체</p>
        </div>
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="grid grid-cols-4 gap-4 mb-8">
              {[
                { label: '강원 워케이션 방문자', value: `${totalVisitors.toLocaleString()}명`, sub: `총 ${active.length}건 예약`,       color: 'text-blue-400'    },
                { label: '참여 기업·개인 수',   value: `${totalCompanies}개사`,               sub: `확정 ${confirmed.length}건`,        color: 'text-emerald-400' },
                { label: '지역 경제 효과',       value: fmtMoney(totalRevenue),               sub: '확정 예약 합산',                    color: 'text-amber-400'   },
            ...
            <div className="grid grid-cols-3 gap-6 mb-6">
              ...
              <div className="col-span-2 bg-white border border-[#E2E8F0] rounded-xl overflow-hidden">
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {cityRows.map(([city, bks]) => {
                      const visitors    = bks.reduce((s, b) => s + (b.guests || 0), 0)
                      const totalNights = bks.reduce((s, b) => s + nightsCount(b.start_date, b.end_date) * (b.guests || 0), 0)
                      const pct         = totalVisitors > 0 ? Math.round((visitors / totalVisitors) * 100) : 0
                      return (
                        <tr key={city} className="border-b border-[#E2E8F0]/50 hover:bg-[#F1F5F9]/50 transition-colors">
                          <td className="px-5 py-3 font-medium">강원 {city}</td>
                          <td className="px-5 py-3 text-blue-400 font-bold">{visitors.toLocaleString()}명</td>
                          <td className="px-5 py-3 text-[#475569]">{bks.length}건</td>
                          <td className="px-5 py-3 text-[#475569]">{totalNights.toLocaleString()}박</td>
                          <td className="px-5 py-3 text-xs text-[#94A3B8]">📍 {CITY_SPOTS[city] ?? '-'}</td>
                    ...
                    {cityRows.length > 0 && (
                      <tr className="bg-[#F8FAFC]">
                        <td className="px-5 py-3 font-bold text-xs text-[#475569]">강원도 합계</td>
                    ...
                    {cityRows.length === 0 && (
                      <tr>
                        <td colSpan={6} className="px-5 py-8 text-center text-sm text-[#94A3B8]">강원도 워케이션 예약이 없어요</td>
                      </tr>
                    )}
                  </tbody>
                </table>
              ...
              <div className="bg-white border border-[#E2E8F0] rounded-xl p-5">
                <div className="font-semibold text-sm mb-5">강원도 월별 방문자 추이</div>
            ...
            <div className="bg-gradient-to-br from-emerald-500/10 to-blue-500/10 border border-emerald-500/20 rounded-xl p-5 mb-6">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="font-bold text-sm">강원도 워케이션 지원금 현황</h2>
              ...
              <div className="grid grid-cols-3 gap-6">
                <div className="bg-white/70 border border-emerald-500/20 rounded-xl p-4">
                  <div className="text-2xl font-black text-emerald-500 mb-1">{GANGWON_SUBSIDY_AMOUNT.toLocaleString()}원</div>
                  <div className="text-xs font-semibold text-[#0F172A] mb-0.5">1인당 지원금 (기본)</div>
                  <div className="text-xs text-[#94A3B8]">2박 이상 · 속초 최대 50만원</div>
                </div>
                <div className="bg-white/70 border border-emerald-500/20 rounded-xl p-4">
                  <div className="text-2xl font-black text-blue-500 mb-1">{fmtMoney(subsidyUsed)}</div>
                  ...
                  <div className="text-xs text-[#94A3B8] mt-1">총 예산 {fmtMoney(GANGWON_BUDGET)} · {budgetPct}% 집행</div>
                </div>
                <div className="bg-white/70 border border-emerald-500/20 rounded-xl p-4">
                  ...
                  <div className="flex gap-2 mt-2">
                    <span className="text-xs bg-emerald-500/20 text-emerald-600 px-2 py-0.5 rounded-full">승인 {confirmed.length}건</span>
                    <span className="text-xs bg-amber-500/20 text-amber-600 px-2 py-0.5 rounded-full">검토중 {pending.length}건</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white border border-[#E2E8F0] rounded-xl overflow-hidden">
              <div className="flex items-center justify-between px-5 py-4 border-b border-[#E2E8F0]">
                <div>
                  <span className="font-semibold text-sm">강원도 워케이션 예약 · 지원금 신청 현황</span>
                  <span className="text-xs text-[#94A3B8] ml-2">1인당 {GANGWON_SUBSIDY_AMOUNT.toLocaleString()}원~ 지원</span>
                </div>
                {pending.length > 0 && (
                  <span className="text-xs text-amber-400 bg-amber-500/10 px-2.5 py-1 rounded-lg">검토중 {pending.length}건</span>
                )}
              </div>

              {bookings.length === 0 ? (
                <div className="py-16 text-center text-sm text-[#94A3B8]">강원도 워케이션 예약이 없어요</div>
              ) : (
                <table className="w-full text-sm">
                  <thead>
                    <tr className="text-xs text-[#94A3B8] border-b border-[#E2E8F0]">
                  ...
                  <tbody>
                    {bookings.map((b) => {
                      ...
                      const subsidyAmt = (b.guests || 0) * GANGWON_SUBSIDY_AMOUNT
                      return (
                            ...
                            {b.status === 'pending' && (
                              <div className="flex gap-1.5">
                                <button onClick={() => updateStatus(b.id, 'confirmed')} disabled={updating}
                                  className="text-xs px-2.5 py-1 bg-emerald-500/20 text-emerald-400 rounded-md hover:bg-emerald-500/30 disabled:opacity-50">승인</button>
                                <button onClick={() => updateStatus(b.id, 'cancelled')} disabled={updating}
                                  className="text-xs px-2.5 py-1 bg-red-500/10 text-red-400 rounded-md hover:bg-red-500/20 disabled:opacity-50">반려</button>
                              </div>
                            )}
                            {b.status !== 'pending' && (
