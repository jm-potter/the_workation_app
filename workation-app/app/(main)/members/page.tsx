    ...
    <div className="min-h-screen bg-[#0F172A]">
      ...
      <div className="max-w-4xl mx-auto px-6 py-8">
        <div className="mb-8">
          ...
          <p className="text-sm text-[#94A3B8]">초대 링크 발송 및 부서·권한 설정</p>
        </div>
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="bg-[#1E293B] border border-[#334155] rounded-2xl p-6">
            ...
            <p className="text-xs text-[#64748B] mb-4">이메일을 입력하면 초대 링크를 일괄 발송합니다 (쉼표 또는 줄 바꿈으로 구분)</p>
            ...
            <div className="flex flex-col gap-3">
              <textarea
                ...
                className="w-full bg-[#263548] border border-[#334155] rounded-xl px-4 py-3 text-sm text-[#F1F5F9] placeholder-[#64748B] focus:outline-none focus:border-blue-500 resize-none font-mono"
              />

              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs text-[#94A3B8] mb-1.5 block">부서 배정</label>
                  <select
                    value={inviteDept}
                ...
                <div>
                  <label className="text-xs text-[#94A3B8] mb-1.5 block">권한</label>
                  <select
                    value={inviteRole}
          ...
          <div className="bg-[#1E293B] border border-[#334155] rounded-2xl p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-bold">멤버 현황 <span className="text-sm font-normal text-[#64748B]">{members.length}명</span></h2>
              <div className="flex items-center gap-3">
                <input
                  value={search}
                  onChange={e => setSearch(e.target.value)}
                  placeholder="이름·이메일 검색"
                  className="bg-[#263548] border border-[#334155] rounded-xl px-3 py-2 text-sm text-[#F1F5F9] placeholder-[#64748B] focus:outline-none focus:border-blue-500 w-44"
                />
                <div className="flex gap-1.5">
                  {DEPTS.map(d => (
                    <button key={d} onClick={() => setDept(d)}
                      className={`px-2.5 py-1 rounded-lg text-xs font-medium transition-colors ${
                        dept === d ? 'bg-blue-500 text-white' : 'bg-[#263548] border border-[#334155] text-[#94A3B8] hover:border-blue-500/50'
                      }`}>
                      {d}
                    </button>
                  ))}
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-[#1E293B]">
                  {filtered.map(m => (
                    <tr key={m.id} className="hover:bg-[#263548]/50 transition-colors">
                      <td className="py-3 pr-4 font-medium">{m.name}</td>
                      <td className="py-3 pr-4 text-[#64748B] font-mono text-xs">{m.email}</td>
                      <td className="py-3 pr-4 text-[#94A3B8]">{m.dept}</td>
                      <td className="py-3 pr-4">
                        <span className={`text-xs px-2 py-0.5 rounded-md ${
                          m.role === 'hr' ? 'bg-blue-500/20 text-blue-300' : 'bg-[#263548] text-[#94A3B8]'
                        }`}>
                      ...
                      <td className="py-3 text-right">
                        {m.status === 'pending' && (
                          <button onClick={() => removeInvite(m.id)} className="text-xs text-[#64748B] hover:text-red-400 transition-colors">
                            취소
                          </button>
                        )}
                      </td>
