    ...
    <div className="min-h-screen bg-[#0F172A]">
      ...
      <div className="max-w-2xl mx-auto px-6 py-10">
        <div className="mb-8">
          ...
          <p className="text-sm text-[#94A3B8]">예상 비용을 미리 계산해보세요</p>
        </div>

        <div className="grid grid-cols-1 gap-5">
          {/* 입력 카드 */}
          <div className="bg-[#1E293B] border border-[#334155] rounded-2xl p-6 flex flex-col gap-5">
            <h2 className="font-bold text-sm text-[#94A3B8] uppercase tracking-wide">조건 입력</h2>

            {/* 숙소 선택 */}
            <div>
              <label className="text-xs text-[#94A3B8] mb-1.5 block">숙소</label>
              <select
                value={accIdx}
            ...
            <div className="grid grid-cols-2 gap-4">
              {/* 숙박 일수 */}
              <div>
                <label className="text-xs text-[#94A3B8] mb-1.5 block">숙박 일수</label>
                <div className="flex items-center gap-3">
                  <button
                    onClick={() => setNights(Math.max(1, nights - 1))}
                    className="w-8 h-8 rounded-lg bg-[#263548] border border-[#334155] text-[#94A3B8] hover:border-blue-500 transition-colors"
                  >-</button>
                  ...
                  <button
                    ...
                    className="w-8 h-8 rounded-lg bg-[#263548] border border-[#334155] text-[#94A3B8] hover:border-blue-500 transition-colors"
                  >+</button>
                  <span className="text-xs text-[#64748B]">박</span>
                </div>
              </div>

              {/* 인원 */}
              <div>
                <label className="text-xs text-[#94A3B8] mb-1.5 block">참여 인원</label>
                <div className="flex items-center gap-3">
                  <button
            ...
            {/* 기타 비용 */}
            <div>
              <label className="text-xs text-[#94A3B8] mb-1.5 block">기타 비용 (팀빌딩, 액티비티 등)</label>
              <div className="relative">
                <input
                  type="number"
                  value={extra}
                  onChange={e => setExtra(Number(e.target.value))}
                  className="w-full bg-[#263548] border border-[#334155] rounded-xl px-3 py-2.5 text-sm text-[#F1F5F9] focus:outline-none focus:border-blue-500 pr-8"
                />
          ...
          <div className="bg-[#1E293B] border border-[#334155] rounded-2xl p-6">
            <h2 className="font-bold text-sm text-[#94A3B8] uppercase tracking-wide mb-4">예상 비용 내역</h2>
              ...
              {[
                ...
                <div key={label as string} className="flex justify-between items-center">
                  <div>
                    ...
                    {desc && <span className="text-xs text-[#64748B] ml-2">{desc}</span>}
                  </div>
          ...
          {/* 예산 비교 */}
          <div className="bg-[#1E293B] border border-[#334155] rounded-2xl p-6">
            <h2 className="font-bold text-sm text-[#94A3B8] uppercase tracking-wide mb-4">회사 예산 현황</h2>
            <div className="flex justify-between text-sm mb-2">
              <span className="text-[#94A3B8]">연간 예산</span>
              <span>5,000,000원</span>
            </div>
            <div className="flex justify-between text-sm mb-2">
              <span className="text-[#94A3B8]">이미 사용</span>
              <span className="text-amber-400">1,850,000원</span>
            </div>
            <div className="flex justify-between text-sm mb-4">
              <span className="text-[#94A3B8]">이번 워케이션 예상</span>
              <span className="text-blue-400 font-bold">{total.toLocaleString()}원</span>
            </div>
            <div className="h-3 bg-[#263548] rounded-full overflow-hidden mb-2">
            ...
            <div className="flex gap-4 text-xs text-[#64748B]">
              ...
              <span className="flex items-center gap-1"><span className="w-2 h-2 bg-[#263548] rounded-full inline-block" />잔여</span>
            </div>
          </div>
        </div>
      </div>
