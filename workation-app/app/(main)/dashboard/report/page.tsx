      ...
      <div className="max-w-4xl mx-auto px-6 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-sm text-[#94A3B8]">삼성전자 · 강릉 씨사이드 · 2026년 6월 10~13일 · 28명 참여</p>
          </div>
          <div className="flex gap-2">
            <button className="text-sm px-4 py-2 bg-[#1E293B] border border-[#334155] rounded-xl hover:border-blue-500/50 transition-colors">
              PDF 저장
            </button>
        ...
        <div className="bg-gradient-to-br from-blue-500/10 to-purple-500/10 border border-blue-500/20 rounded-2xl p-6 mb-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="text-xs bg-blue-500/20 text-blue-300 px-2 py-0.5 rounded-full border border-blue-500/30">AI 생성</span>
          </div>
          ...
          <div className="bg-[#1E293B] rounded-xl p-4 text-sm leading-relaxed text-[#94A3B8]">
            <p>
              이번 강릉 워케이션(6/10~13, 28명)은 <strong className="text-white">직원 만족도 +18%p, 팀 결속력 +19%p</strong> 향상을 이끌어냈습니다.
              특히 이직 의향이 <strong className="text-emerald-400">38% → 19%로 절반 수준으로 감소</strong>하여 인재 유지 비용 절감 효과가 두드러집니다.
              투자 비용 대비 생산성·리텐션 효과를 종합한 <strong className="text-white">ROI는 3.2배</strong>로 측정됐으며,
        ...
        <div className="bg-[#1E293B] border border-[#334155] rounded-2xl p-6 mb-6">
          ...
          <div className="grid grid-cols-3 gap-4">
            {BEFORE_AFTER.map(p => {
              ...
              return (
                <div key={p.label} className="bg-[#263548] rounded-xl p-4">
                  ...
                  <div className="flex items-end gap-3 mb-3">
                    <div className="flex-1 text-center">
                      ...
                      <div className="h-20 bg-[#1E293B] rounded-lg flex items-end overflow-hidden">
                        <div className="w-full bg-[#334155] rounded-lg" style={{ height: `${p.before}%` }} />
                      </div>
                      <div className="text-sm font-bold mt-1 text-[#94A3B8]">{p.before}{p.unit}</div>
                    </div>
                    <div className="text-[#64748B] mb-4">→</div>
                    <div className="flex-1 text-center">
                      <div className="text-xs text-[#64748B] mb-1">이후</div>
                      <div className="h-20 bg-[#1E293B] rounded-lg flex items-end overflow-hidden">
                        <div className="w-full bg-blue-500/70 rounded-lg" style={{ height: `${p.after}%` }} />
                      </div>
                      <div className="text-sm font-bold mt-1 text-blue-300">{p.after}{p.unit}</div>
                    </div>
                  </div>
        ...
        <div className="bg-gradient-to-br from-emerald-500/10 to-blue-500/10 border border-emerald-500/20 rounded-2xl p-6 mb-6">
          ...
          <div className="bg-[#1E293B] rounded-xl p-4 text-xs text-[#64748B] leading-relaxed">
            <strong className="text-[#94A3B8]">산출 기준:</strong> 평균 통근 거리 {CARBON.avgCommute}km × 왕복 × {CARBON.days}일 × {CARBON.people}명 × 0.21kgCO₂/km = <strong className="text-emerald-400">{carbonSaved}kg CO₂</strong> 저감.
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              <div key={t.name} className={`rounded-xl p-4 border ${
                t.status === 'connected' ? 'bg-[#263548] border-[#334155]' : 'bg-[#1a2540] border-[#334155] opacity-60'
              }`}>
                ...
                <div className="text-xs text-[#64748B]">{t.data}</div>
              </div>
            ))}
          </div>
          <Link href="/settings" className="mt-4 block text-center text-xs text-blue-400 hover:text-blue-300 transition-colors">
            협업툴 연동 설정 →
          </Link>
        </div>
      </div>
