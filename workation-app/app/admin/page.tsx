  ...
  return (
    <div className="min-h-screen bg-[#0F172A]">
      ...
      <header className="sticky top-0 z-50 bg-[#0F172A]/90 backdrop-blur border-b border-[#334155] px-6 h-14 flex items-center justify-between">
        ...
        <div className="text-sm text-[#64748B]">운영팀 · 관리자</div>
      </header>
        ...
        <div className="grid grid-cols-4 gap-4 mb-8">
          {STATS.map((s) => (
            <Link key={s.label} href={s.href}
              className="bg-[#1E293B] border border-[#334155] rounded-xl p-5 hover:border-blue-500/50 transition-colors">
        ...
        <div className="grid grid-cols-3 gap-6">
          ...
          <div className="col-span-2 bg-[#1E293B] border border-[#334155] rounded-xl overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-[#334155]">
              ...
              <Link href="/admin/bookings" className="text-xs text-blue-400 hover:text-blue-300">전체 보기 →</Link>
            </div>
              ...
              <tbody>
                {RECENT_BOOKINGS.map((b) => (
                  <tr key={b.id} className="border-b border-[#334155]/50 hover:bg-[#263548]/50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-xs text-[#94A3B8]">{b.company}</td>
                    <td className="px-4 py-3 font-medium text-sm">{b.employee}</td>
                    <td className="px-4 py-3 text-xs text-[#94A3B8]">{b.accommodation}</td>
          ...
          <div className="bg-[#1E293B] border border-[#334155] rounded-xl overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-[#334155]">
              ...
              <Link href="/admin/accommodations" className="text-xs text-blue-400 hover:text-blue-300">전체 →</Link>
            </div>
            <div className="flex flex-col divide-y divide-[#334155]">
              {PENDING_ACCOMMODATIONS.map((a) => (
                <div key={a.name} className="px-5 py-4 hover:bg-[#263548]/50 transition-colors cursor-pointer">
                  <div className="font-medium text-sm mb-1">{a.name}</div>
                  <div className="text-xs text-[#64748B] mb-2">📍 {a.location}</div>
        ...
        <div className="grid grid-cols-4 gap-4 mt-6">
          {[
            ...
            <Link key={m.label} href={m.href}
              className="bg-[#1E293B] border border-[#334155] rounded-xl p-4 flex items-center gap-3 hover:border-blue-500/50 transition-colors">
